...
| 동시 사용자 | 자동으로 서버가 늘어나는 구조가 아님 | 부하가 지속되면 `t3.large` 또는 DB 서버 증설 필요 |
| DB 읽기·쓰기 | 쿼리당 별도 요금 없음 | EBS IOPS·처리량 초과 설정, 디스크 증설, DB 서버 증설 |
| 로그·모니터링 | 현재 CloudWatch 비용은 0달러 수준 | 로그·메트릭 수집량이 커지거나 보관기간을 늘릴 때 |

현재 ANASA는 Auto Scaling이나 RDS를 사용하지 않으므로 순간 요청량이 곧바로 인스턴스 추가 비용으로 이어지지는 않는다. 대신 CPU·메모리·디스크 지연이 먼저 증가한다. 메모리 부족으로 앱 서버를 `t3.medium`에서 `t3.large`로 변경하면 월 **$42.56 / 약 6만원**, 연 **$510.68 / 약 72만원**이 추가된다. DB 서버를 8 vCPU로 증설하면 DB EC2와 SQL Server 코어 라이선스가 함께 증가하므로 별도 견적이 필요하다.
...
| 앱 서버 `t3.large` 전환 | 변동 없음 | **+$42.56/월 / 약 6만원** | 메모리 4GiB → 8GiB, vCPU는 동일 |
...
기준 금액은 현재 DB의 3배, 앱 `t3.medium`, SQL Server Standard 연간 구독을 적용한 월 **$688 / 약 96만원**이다.

| 최적화 수준 | 포함 범위 | 월 증가액(기본 96만원 대비) | 예상 총 월 비용(VAT 포함) |
| --- | --- | ---: | ---: |
| 무증설 최적화 | Query Store, 실행계획, 쿼리 수정, 기존 Redis 캐시 | **+$0 / +0원** | **$688 / 약 96만원** |
| 인덱스 중심 | 무증설 최적화 + 인덱스용 EBS 0–300GB | **+$0–30 / +0–4.2만원** | **약 $688–718 / 96–100만원** |
| 균형형 | 인덱스, `t3.large` 또는 단일 Redis, 필요 시 gp3 성능 보강 | **약 +$71–129 / +10–18만원** | **약 $759–817 / 106–114만원** |
| 고성능형 | Redis HA, 높은 gp3 성능, DB 8 vCPU·64GiB 증설 | **약 +$643–750 / +90–105만원** | **약 $1,331–1,438 / 186–201만원** |

위 범위는 선택 옵션을 단계별 패키지로 본 계획값이다. 모든 개별 추가비용을 무조건 합산한 값이 아니며, 부하 테스트 후 필요한 단계까지만 적용한다. 인덱스와 캐시만으로 목표 응답시간을 달성하면 월 비용은 약 96–114만원 범위에 머물 수 있고, DB 서버 증설까지 필요할 때 월 약 186–201만원까지 증가할 수 있다.
...
| --- | ---: | --- |
| 기존 `t3.medium` 유지 | **약 6만원/월** | `t3.large` 메모리 증설 방지 |
...
따라서 최적화의 우선 목표는 현재 월 약 96만원을 더 낮추는 것보다, 향후 월 106만–201만원 수준으로 증가하는 것을 방지하는 데 있다. 실제 비용을 현재보다 낮추려면 최적화 후 서버 다운사이징, 별도 캐시 제거, EBS 축소 가능 여부 등을 별도로 검토해야 한다.
